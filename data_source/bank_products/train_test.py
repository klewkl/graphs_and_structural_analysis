...
def train(model, data, optimizer):
    ...
    data.neg_edge_index_u2a = negative_sampling(
        ...
        num_nodes=(data.num_customers, data.num_services),  # number of nodes
        num_neg_samples=data.edge_index_u2a.size(1),
        method='sparse').to('cuda')  # number of neg_sample equal to number of pos_edges

    optimizer.zero_grad()

    z_customers, z_services = model.forward()  # encode
    loss = model.BPRLoss(model.decode_all(z_customers, z_services),
                         to_scipy_sparse_matrix(data.edge_index_u2a, num_nodes=(data.num_customers, data.num_services)).toarray(),
                         data.edge_index_u2a)
