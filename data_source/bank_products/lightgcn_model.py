...
class LightGCNStack(torch.nn.Module):
    def __init__(self, latent_dim, args):
        ...
        self.dataset = None
        self.embeddings_customers = None
        self.embeddings_services = None
    ...
    def init_data(self, dataset):
        ...
        self.embeddings_customers = torch.nn.Embedding(num_embeddings=dataset.num_customers, embedding_dim=self.latent_dim).to('cuda')
        self.embeddings_services = torch.nn.Embedding(num_embeddings=dataset.num_services, embedding_dim=self.latent_dim).to('cuda')

    def forward(self):
        x_users, x_artists, batch = self.embeddings_customers.weight, self.embeddings_services.weight, \
                                                self.dataset.batch

        embeddings_customers = torch.zeros(size=x_users.size(), device='cuda')
        embeddings_services = torch.zeros(size=x_artists.size(), device='cuda')
        embeddings_customers = embeddings_customers + x_users/(self.num_layers + 1)
        embeddings_services = embeddings_services + x_artists/(self.num_layers+1)
        for i in range(self.num_layers):
            x_users = self.convs[i]((x_artists, x_users), self.dataset.edge_index_a2u, size=(self.dataset.num_services, self.dataset.num_customers))
            x_artists = self.convs[i]((x_users, x_artists), self.dataset.edge_index_u2a, size=(self.dataset.num_customers, self.dataset.num_services))
            embeddings_customers = embeddings_customers + x_users/(self.num_layers+1)
            embeddings_services = embeddings_services + x_artists/(self.num_layers + 1)

        return embeddings_customers, embeddings_services
